# 憎恶地窟：可选脚本兼容补丁（extra 1.0.50）

> 本版沿用 1.0.49 的同一份七文件补丁，并补充 PF2e 8.5.1 代表入口验证；实际验证边界见下文。

此附件用于 **Abomination Vaults Addons 2.1.0** 与 **Abomination Vaults Expanded 3.2.0**。它只修改这两个已安装模块的 7 个脚本；不包含第三方模块完整源码、冒险包、素材、世界数据或备份。请自行安装原模块及配套 **pf2e-compendium-extra-cn 1.0.50**。本附件不是可导入 Foundry 的模块。

已应用 1.0.49 附件的用户：本版补丁内容及 7 个修补后文件的 SHA 与 1.0.49 相同。更新 extra 后，用本附件的 `verify.py --modules "实际 Data/modules 路径" --expect after` 核对；全部通过即可保留现有脚本，无需再次应用补丁。校验失败时按下方更新边界处理。
...
验证环境：Foundry **14.367**、PF2e **8.5.0 / 8.5.1**、Babele **2.9.1**、核心汉化 **3.1.2**，以及上述两个模块的确切版本。补丁内仍保留中文语言、模块版本和实体来源限制。

8.5.0 复用 1.0.49 的同字节验证。本次 8.5.1 原生检查已通过开场选择、辅助提示、场景设置、贝科拉之握和拉杆入口；中文日志切片插入及稳定身份布景创建也实际成功，且临时数据清理恢复。后两项因隔离测试未附带的图片或样式返回 404，整项严格检查未通过，不能视为完整界面验收。本次没有重验完整媒体、音频、动画或贝科拉攻击的十字定位交互。
...
**Git 退出码为 0 不代表补丁已应用。** 如果 `Data/modules` 位于外层 Git 仓库的子目录中，Git 可能跳过全部补丁路径而仍返回 0。下方命令只在相关 Git 调用期间，以已解析的 `modules` 父目录设置 `GIT_CEILING_DIRECTORIES`，阻止发现外层仓库；不会修改 Git 配置。应用后必须运行 `--expect after`，确认 7 个文件全部匹配并显示 `PASS`；反向恢复后必须运行 `--expect before`。任一校验失败都保持停服。
...
```powershell
$AvCompat = (Resolve-Path -LiteralPath 'C:\Downloads\av-source-compat-v1.0.50' -ErrorAction Stop).Path
$AvModules = (Resolve-Path -LiteralPath 'C:\FoundryData\Data\modules' -ErrorAction Stop).Path
$AvGitCeiling = (Get-Item -LiteralPath $AvModules -ErrorAction Stop).Parent.FullName

python "$AvCompat\verify.py" --modules "$AvModules" --expect before
if ($LASTEXITCODE -ne 0) { throw '原版 SHA/版本校验失败，停止。' }

$AvHadGitCeiling = Test-Path Env:GIT_CEILING_DIRECTORIES
$AvOldGitCeiling = $env:GIT_CEILING_DIRECTORIES
try {
    $env:GIT_CEILING_DIRECTORIES = $AvGitCeiling
    ...
    if ($AvHadGitCeiling) { $env:GIT_CEILING_DIRECTORIES = $AvOldGitCeiling }
    else { Remove-Item Env:GIT_CEILING_DIRECTORIES -ErrorAction SilentlyContinue }
}
...
```bash
AV_COMPAT=$(cd -- '/path/to/av-source-compat-v1.0.50' && pwd -P) &&
AV_MODULES=$(cd -- '/path/to/FoundryData/Data/modules' && pwd -P) &&
AV_GIT_CEILING=$(dirname -- "$AV_MODULES") &&
python3 "$AV_COMPAT/verify.py" --modules "$AV_MODULES" --expect before &&
GIT_CEILING_DIRECTORIES="$AV_GIT_CEILING" git -c core.autocrlf=false -C "$AV_MODULES" apply --check --unidiff-zero "$AV_COMPAT/av-source-compat.patch" &&
GIT_CEILING_DIRECTORIES="$AV_GIT_CEILING" git -c core.autocrlf=false -C "$AV_MODULES" apply --unidiff-zero "$AV_COMPAT/av-source-compat.patch" &&
...
AV_GIT_CEILING=$(dirname -- "$AV_MODULES") &&
python3 "$AV_COMPAT/verify.py" --modules "$AV_MODULES" --expect after &&
GIT_CEILING_DIRECTORIES="$AV_GIT_CEILING" git -c core.autocrlf=false -C "$AV_MODULES" apply --check --reverse --unidiff-zero "$AV_COMPAT/av-source-compat.patch" &&
GIT_CEILING_DIRECTORIES="$AV_GIT_CEILING" git -c core.autocrlf=false -C "$AV_MODULES" apply --reverse --unidiff-zero "$AV_COMPAT/av-source-compat.patch" &&
python3 "$AV_COMPAT/verify.py" --modules "$AV_MODULES" --expect before
```
